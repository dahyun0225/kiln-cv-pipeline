...
print(f"VERSION 4: EfficientNetV2-S | Batch={BATCH_SIZE}, Epochs={EPOCHS}")
#<----
...
full_dataset = KilnDataset(CSV_FILE, DATA_DIR, transform=None)
...
train_labels = [df.iloc[i]['label'] for i in train_indices.indices]
...
for epoch in range(EPOCHS):
    ...
    print(f"  Train - Loss: {train_loss/len(train_loader):.4f}, Acc: {train_acc:.2f}%, AUC: {train_auc:.4f}")
    print(f"  Val   - Loss: {val_loss/len(val_loader):.4f}, Acc: {val_acc:.2f}%, AUC: {val_auc:.4f}")
    ...
    if val_auc > best_val_auc:
        ...
        torch.save({
            ...
            'val_auc': val_auc,
            'val_acc': val_acc,
        }, 'best_kiln_model_v4_efficientnet.pth')
        print(f"  new best (AUC: {val_auc:.4f}, Acc: {val_acc:.2f}%)")
    else:
        patience_counter += 1
...
checkpoint = torch.load('best_kiln_model_v4_efficientnet.pth')
...
print("\nClassification note:")
print(classification_report(val_labels_list, val_preds, target_names=['Not Kiln (0)', 'Kiln (1)']))
print(f"Final AUC: {roc_auc_score(val_labels_list, val_probs):.4f}")
#<----
...
if Path(TEST_DIR).exists():
    ...
    df_pred.to_csv("submission4_efficientnet1.csv", index=False) #99.686

    print(f"\nPredictions saved to submission4_efficientnet1.csv")
else:
